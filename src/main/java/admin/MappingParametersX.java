 ...
 * @since 4.0
 */
public class MappingParametersX {
	 ...
	 */
	public static MappingParametersX from(Annotation annotation) {

		Assert.notNull(annotation, "annotation must not be null!");

		if (annotation instanceof Field) {
			return new MappingParametersX((Field) annotation);
		} else if (annotation instanceof InnerField) {
			return new MappingParametersX((InnerField) annotation);
		} else {
			throw new IllegalArgumentException("annotation must be an instance of @Field or @InnerField");
		}
	}

	private MappingParametersX(Field field) {
		index = field.index();
		store = field.store();
	...
	private MappingParametersX(InnerField field) {
		index = field.index();
		store = field.store();
		fielddata = field.fielddata();
		type = field.type();
